...
(function($){
    ...
    var settings = $.extend({
			...
			tpl: {
				...
				overview:		'<div id="feedback-overview"><div class="feedback-logo">Feedback</div><div id="feedback-overview-description"><div id="feedback-overview-description-text"><h3>Description</h3><h3 class="feedback-additional">Additional info</h3><div id="feedback-additional-none"><span>None</span></div><div id="feedback-browser-info"><label title="The browser name, version, and list of installed plugins will be sent to server"><input type="checkbox" name="input-browser-info" id="input-browser-info" checked /> Browser Info</label></div><div id="feedback-page-info"><label title="the page URL you are viewing will be sent to server"><input type="checkbox" name="input-page-info" id="input-page-info" checked /> Page Info</label></div><div id="feedback-page-structure"><label title="the html source code of the page you are viewing will be sent to server"><input type="checkbox" name="input-page-structure" id="input-page-structure" checked /> Page Structure</label></div></div></div><div id="feedback-overview-screenshot"><h3>Screenshot</h3></div><div class="feedback-buttons"><button id="feedback-submit" class="feedback-submit-btn feedback-btn-blue">Submit</button><button id="feedback-overview-back" class="feedback-back-btn feedback-btn-gray">Back</button></div><div id="feedback-overview-error"></div><div class="feedback-wizard-close"></div></div>',
		...
		if (supportedBrowser) {
			...
			$(document).on('click', settings.feedbackButton, function(){
					...
					html2canvas($('body'), {
						onrendered: function(canvas) {
							...
							if(settings.showDescriptionModal) {
								...
								$('<textarea id="feedback-overview-note" placeholder="Provide a brief description of the issue you are facing">' + $('#feedback-note').val() + '</textarea>').insertAfter('#feedback-overview-description-text h3:eq(0)');
								$('#feedback-overview-screenshot').append('<img class="feedback-screenshot" src="' + img + '" />');							
                                if(settings.requireContactInfo) {
                                    $('<h3>Contact Info</h3><input type="text" id="feedback-overview-contact" placeholder="Your email or phone number so that we can get back to you" />').insertAfter('#feedback-overview-note');
                                }
							}
							else {
				...
				$(document).on('click', '#feedback-submit', function() {
                    ...
                    if (settings.requireContactInfo == true && $('#feedback-overview-contact').val().length > 0) {
                        post.contact = $('#feedback-overview-contact').val();
					    if ($('#feedback-note').val().length > 0) {
						    ...
						    $('#feedback-overview-error').text('Please enter a description.').show();
					    }
                    } else {
                        $('#feedback-overview-error').text('Please enter your contact info.').show();
                    }
				});
			});
		}
